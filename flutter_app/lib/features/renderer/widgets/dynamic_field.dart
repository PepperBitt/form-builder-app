...
class _DynamicFieldState extends State<DynamicField> {
  ...
  /// Handles the file picking and uploading flow.
  Future<void> _pickAndUploadFile(FormFieldState state) async {
    try {
      ...
      final uploadResult = await _uploadService.uploadFile(
        filePath: kIsWeb ? null : pickedFile.path, // null on web
        fileBytes: pickedFile.bytes, // available on web (and when withData: true)
        fileName: fileName,
      );
      ...
      state.didChange(uploadResult.url);
    } catch (e) {
      setState(() {
        _isUploading = false;
  ...
  void _removeUploadedFile(FormFieldState state) {
    setState(() {
      _uploadedFileUrl = null;
        ...
        if (field.isRequired) {
          switch (field.type) {
            ...
            case FieldType.checkbox:
              if (_selectedCheckboxes.isEmpty) return 'Please select at least one option';
              break;
            case FieldType.fileUpload:
              if (_uploadedFileUrl == null) return 'Please upload a file';
              break;
          }
        }
        return null;
      ...
      onSaved: (v) {
        switch (field.type) {
          ...
          case FieldType.date:
            ...
            break;
          case FieldType.fileUpload:
            widget.onSaved?.call(_uploadedFileUrl);
            break;
        }
      },
      builder: (state) {
  ...
  Widget _buildInput(FieldModel field, FormFieldState state) {
    switch (field.type) {
      ...
      case FieldType.fileUpload:
        return _buildFileUpload(state);
    }
  }

  /// Builds the file upload widget with idle / uploading / uploaded / error states.
  Widget _buildFileUpload(FormFieldState state) {
    // --- Uploading state ---
    if (_isUploading) {
      return Container(
      ...
      return Container(
        ...
        child: Row(
          children: [
            ...
            IconButton(
              onPressed: () => _removeUploadedFile(state),
    ...
    if (_uploadError != null) {
      return GestureDetector(
        onTap: () => _pickAndUploadFile(state),
        child: Container(
          padding: const EdgeInsets.all(16),
          decoration: BoxDecoration(
